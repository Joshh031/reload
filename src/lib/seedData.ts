import type { Minutes, Place } from './types';

// Seed content lives in its own module (no imports beyond types) so both the
// seeder and the demo-data-removal migration can reference it without cycles.
...
export const HUES = [210, 0, 30, 120, 280, 45, 170, 90, 330];
...
export const SEED: Record<string, SeedCard[]> = {
  Fund: [
    {
...
export const SEED_ACTIONS: ReadonlySet<string> = new Set(
  Object.values(SEED)
    .flat()
    .map((s) => s.action)
);
